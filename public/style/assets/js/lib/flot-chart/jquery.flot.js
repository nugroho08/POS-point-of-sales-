...
(function($) {
    ...
    $.color.parse = function(str) {
        ...
        if (
            (res = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(
                str
            ))
        )
            return m(
...
(function($) {
    ...
    Canvas.prototype.removeText = function(layer, x, y, text, font, angle) {
        if (text == null) {
            ...
            var positions = this.getTextInfo(layer, text, font, angle)
                .positions;
            for (var i = 0, position; (position = positions[i]); i++) {
                if (position.x == x && position.y == y) {
                    position.active = false;
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        var series = [],
            options = {
                ...
                xaxis: {
                    ...
                    ticidecimals: null, // no. of decimals, null means auto
        ...
        plot.getAxes = function() {
            ...
            $.each(xaxes.concat(yaxes), function(_, axis) {
                if (axis)
                    res[
                        axis.direction + (axis.n != 1 ? axis.n : "") + "axis"
                    ] = axis;
            });
            return res;
        };
        ...
        function setupGrid() {
            ...
            for (var a in plotOffset) {
                ...
                plotOffset[a] =
                    typeof margin == "number" ? margin : margin[a] || 0;
            }
        ...
        function setupTickGeneration(axis) {
            ...
            axis.delta = delta;
            axis.ticidecimals = Math.max(0, maxDec != null ? maxDec : dec);
            ...
            if (!axis.tickGenerator) {
                ...
                axis.tickFormatter = function(value, axis) {
                    var factor = axis.ticidecimals
                        ? Math.pow(10, axis.ticidecimals)
                        : 1;
                    var formatted = "" + Math.round(value * factor) / factor;

                    // If ticidecimals was specified, ensure that we have exactly that
                    // much precision; otherwise default to the value's own precision.

                    if (axis.ticidecimals != null) {
                        var decimal = formatted.indexOf(".");
                        var precision =
                            decimal == -1 ? 0 : formatted.length - decimal - 1;
                        if (precision < axis.ticidecimals) {
                            return (
                                (precision ? formatted : formatted + ".") +
                                ("" + factor).substr(
                                    1,
                                    axis.ticidecimals - precision
                                )
                            );
                        }
                    }
            ...
            if (opts.alignTicksWithAxis != null) {
                ...
                if (otherAxis && otherAxis.used && otherAxis != axis) {
                    ...
                    if (!axis.mode && opts.ticidecimals == null) {
                        var extraDec = Math.max(
                                0,
                                -Math.floor(Math.log(axis.delta) / Math.LN10) +
                                    1
                            ),
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (
                        ...
                        (t == "full" &&
                            ((typeof bw == "object" && bw[axis.position] > 0) ||
                                bw > 0) &&
